...
export const getLogs = async (tag) => {
  ...
  return (await TABLE.logs.select(options).all())
    .map((record) => {
...
export const deleteAdmin = async (email) => {
  ...
  if (records.length === 1) throw "Must have at least 1 admin"
  const record = await getRecord(TABLE.admins, ["email", email])
  return await TABLE.admins.destroy(record.id)
}

export const updateAdmin = async ({ email, phone }) => {
  const record = await getRecord(TABLE.admins, ["email", email])
  return adminSubset(await TABLE.admins.update(record.id, { phone }))
}

export const createAdmins = pluralizer(createAdmin)

// create a single log
export const createLog = async ({ tag, body, date, id }) => {
  return logSubset(
    await TABLE.logs.create({
      tag,
      body,
      date,
      id
...
export const updateLog = async ({
  id,
  body: { total, queued, failed, sent, delivered, undelivered, data, data2 }
}) => {
  const record = await getRecord(TABLE.logs, ["id", id])
  return logSubset(
    await TABLE.logs.update(record.id, {
      total,
      queued,
      failed,
      sent,
