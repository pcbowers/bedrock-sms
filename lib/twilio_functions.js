...
export const getBindingID = async (identity) => {
...
export const listContact = async (identity) => {
  const bindingID = await getBindingID(identity)
...
// get history of contact
export const historyContact = async (identity) => {
  const { address: number } = await listContact(identity)
  return [
...
// delete a contact
export const deleteContact = async (identity) => {
  const bindingID = await getBindingID(identity)
...
export const updateContact = async ([tags = [], identity]) => {
  const { address: number } = await listContact(identity)
  return await service.bindings.create({
    identity: identity,
    bindingType: 'sms',
...
export const patchContact = async ([newTags = [], identity]) => {
  const { address: number, tags: oldTags } = (await listContact(identity))
  return await service.bindings.create({
    identity: identity,
    bindingType: 'sms',
